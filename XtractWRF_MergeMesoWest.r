...
#Based on 'XtractHRRR_MergeMesoWestV2.r'
...
metdir<-"/uufs/chpc.utah.edu/common/home/lin-group5/WRF_files/CO2_USA/sept_2015/"
mettype<-"WRF"
...
metresultname<-paste0(mettype,"_",start,"to",end,".RDS")
if(XtractTF){
...
#Since wind is grid relative, use read_met_wind instead of read_met to access wind fields. This is called in the same way as read_met, only no var argument is required. This returns a rasterStack with u and v layers that have been rotated into +proj=longlat.
  ...
  #X11();plot(uv$v,zlim=c(-10,10),main=paste("Vwind; lvl =",nlvl,"\n",metfile))
  ...
  cn<-cellFromXY(uv$u,coords[,c("LON","LAT")])   #cell number coinciding with lon/lat coordinates
  #xyFromCell(uv$u,cellFromXY(uv$u,coords[,c("LON","LAT")]))   #'xyFromCell' is the opposite of 'cellFromXY'
  #!!!! if want to carry out interpolation, use 'fourCellsFromXY' instead !!!!#
  Usim<-uv$u[cn]; Vsim<-uv$v[cn]
  Tsim<-Tair[cn] - 273.15   #[K]=>[C]
...
finalresultname<-paste0(mettype,"_obs_",start,"to",end,".RDS")
